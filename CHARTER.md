...
6. **時空意圖精確配對 (Intent-based Time Matching)**：查詢系統（尤其是課表查詢）必須具備基礎的時間意圖判斷能力。當使用者明確提及相對時空（如：下週、上週）或絕對時空（如：3/5 課表）時，系統**絕對必須**以推算出的精確目標時間去對應與提取資料，並禁止系統在「找不到指定時空」的情況下，擅自把「當前時空或最新的一份」舊資料塞回給使用者造成混淆。
...
2. **靜默忽略貼圖 (Ignore Stickers)**：不論是一對一或群組對話，針對使用者發送的 LINE 貼圖 (`type: 'sticker'`)，機器人**絕對必須**保持靜默 (回傳 `Promise.resolve(null)`)。嚴禁對貼圖發送任何罐頭訊息或無意義的互動，避免佔用 API 資源與造成洗版。
3. **無禮貌防護 (Polite Buffer)**：不論是否被觸發，遇到含有攻擊性或髒話的輸入，一律轉換為教練的詼諧建議（例：「Banana！看來你今天火氣有點大，要不要先去跑個 10K 降降火？」）。
4. **字數節制限制 (Concise Limit)**：為了維持 LINE 對話框的美觀，非必要情況下，AI 產生的回覆文字請縮減在 150 字以內，採用列點或分段。針對餐廳或跑鞋等詳細介紹，強烈建議使用 LINE Flex Message 呈現。
5. **無關圖片過濾指令 (Noise Filter)**：若使用者在群組發送圖片但未標記教練，系統應暗中加上指令強制要求 AI 判斷該圖是否與運動紀錄有關，若無關則單純回傳 `[IGNORE]` 短代碼。主程式接收到該代碼後必須立刻銷毀且不送出任何推播。
6. **有溫度的針對性分析 (Targeted Coaching)**：要求 AI 在處理跑錶截圖或分享數據時，具備截圖人名識別能力。分析語氣必須明確針對「截圖內紀錄的主人翁」進行指教，用充滿溫度、同理心與幽默感的專業用語給予鼓勵，而非盲目地將所有數據均視為傳圖者本人的成績，更絕對不可使用過度貶低或毒舌的用詞。
