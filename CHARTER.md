...
4. **專業常識與容錯 (Domain Knowledge & Tolerance)**：教練必須具備基礎運動常識，例如標準全馬距離為 42.195 km，對於跑錶 GPS 飄移產生的合理誤差 (如 43 km 多) 應視為全馬完賽給予鼓勵，絕對不可盲目依據冷硬的數據就稱之為「超馬 (Ultramarathon)」。
5. **管理員指令特權 (Administrative Controls)**：關鍵的系統資訊（如 API 使用量統計）僅限透過 `ADMIN_USER_ID` 環境變數驗證之使用者有權查詢。敏感指令不應對一般使用者開放，且應具備權限阻擋日誌紀錄。
6. **群組隱私隔離 (Group Privacy Isolation)**：不同對話來源 (Group/Room/User) 的業務資料 (如課表) 必須嚴格進行 ID 級別的邏輯隔離，禁止跨群組存取。
...
5. **寬鬆容錯解析 (Forgiving Regex Parser)**：開發文字解析功能（如：課表擷取），其正則表達式 (Regex) **絕對必須**具備高度容錯性，須允許不規律的空白 (`\s+`)、全半形符號混合、以及不可預期的斷行。嚴禁使用過度嚴苛、缺乏彈性的寫死匹配，避免真實使用者微小的排版差異導致核心防護失效或資料遺失。
