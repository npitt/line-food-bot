...
const { parseUserIntent } = require('./gemini');
...
async function handleMessage(event, client, api) {
  ...
  if (text === '說明' || text === 'help' || text === '？' || text === '?') {
    return client.replyMessage(event.replyToken, {
      type: 'text',
      text: '🍽 使用方式：\n\n1. 傳送「地點名稱或地址」，例如：台北車站、信義威秀\n2. 或傳送你的「位置」（LINE 的定位）\n3. 選擇喜好：料理類型等（可略過）\n4. 點「直接推薦」取得附近餐廳\n\n使用免費地圖資料，點「在地圖開啟／導航」可用 Google 地圖導航。若有設定 Google AI Studio API，可用自然語描述地點與料理類型。',
    });
  }
