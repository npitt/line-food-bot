...
async function handleMessage(event, client, blobClient) {
  ...
  if (event.message.type === 'location') {
    ...
    if (stravaMatch) {
      ...
      promptText = text;
      if (stravaStats) {
        systemContextText = `${stravaStats}\n\n【重要指令】：分析以上用戶傳來的 Strava 運動數據，明確針對「他/該位跑者」進行數據分析（如距離、配速等）。請發揮史都華教練的毒舌與嚴厲性格給予指導或吐槽。絕對不要把這份數據當成是你自己的！`;
      }
    } else {
      // 判斷是否在詢問特定地點的餐廳
    ...
    try {
      ...
      // 處理圖片附帶的文字訊息 (可能會夾帶地點詢問)
      // 若使用手機 LINE 傳送圖片時，也可以同時附帶文字，會被放在 event.message.text
      ...
      // 如果圖片文字含有找尋餐廳意圖，則覆蓋 Prompt
      if (textWithImage || realRestaurantsStr) {
        promptText += `\n\n[使用者附著帶這張圖片問了以下內容]：${textWithImage}`;

        if (realRestaurantsStr) {
          systemContextText = `${realRestaurantsStr}\n\n【重要指令】：針對使用者想找的美食，請你「唯一且絕對必須」從以上提供的【真實世界餐廳清單】中揀選推薦，不要自己憑空捏造名單！如果清單為空，請回報找不到這附近營業中的好餐廳。`;
        }
      }
