...
const { isTrainingSchedule, parseSchedule, formatGroupResult, buildGroupQuickReply, cacheSchedule, getCachedSchedule, isGroupSelection, getLatestSchedule } = require('./schedule');
...
async function handleMessage(event, client, blobClient) {
  ...
  if (event.message.type === 'location') {
    ...
    // --- 2. 檢查是否為「課表」查詢指令 (改為模糊匹配，支援「有這週課表嗎」) ---
    const isScheduleQuery = (text.includes('課表') || text.includes('訓練表')) && text.length < 15;

    if (isScheduleQuery) {
      // 1. 優先查記憶體快取 (這份最快)
      let parsed = getThisWeekSchedule(sourceId) || getLatestSchedule(sourceId);
      ...
      if (!parsed) {
        ...
        for (let i = history.length - 1; i >= 0; i--) {
          ...
          if (isTrainingSchedule(pastText)) {
            parsed = parseSchedule(pastText);
            if (parsed) {
              console.log('[課表救援] 從歷史紀錄成功救回其所屬來源之課表:', parsed.weekLabel);
              cacheSchedule(sourceId, parsed); // 放回該來源的快取
              break;
            }
          }
        }
      ...
      if (parsed) {
        // ... (原有的回覆邏輯)
        const isThisWeek = isDateInPeriod(parsed.periodStr); // Assuming isDateInPeriod is defined elsewhere or imported
        const promptIntro = isThisWeek
          ? `Bello! 🍌 幫你找到本週 (${parsed.periodStr}) 的課表了！`
          : `Bello! 🍌 目前還沒有本週的課表，但教練從紀錄中幫你挖出最近一份 (${parsed.weekLabel})。`;
