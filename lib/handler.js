...
async function handleMessage(event, client, blobClient) {
  ...
  if (event.message.type === 'location') {
    ...
    // --- 2. 檢查是否為「課表」查詢指令 ---
    if (text === '課表' || text === '查課表') {
      const thisWeek = getThisWeekSchedule();
      const latest = getLatestSchedule();

      if (thisWeek) {
        const quickReplyItems = buildGroupQuickReply(thisWeek.groups);
        return sendMessage({
          type: 'text',
          text: `Bello! 🍌 幫你找到本週 (${thisWeek.periodStr}) 的課表了！\n\n請選擇組別 👇`,
          quickReply: { items: quickReplyItems }
        });
      } else if (latest) {
        const quickReplyItems = buildGroupQuickReply(latest.groups);
        return sendMessage({
          type: 'text',
          text: `Bello! 🍌 目前還沒有本週的課表，但教練幫你找到最近一份貼出的課表 (${latest.weekLabel})。\n\n你要看這份嗎？或請提供本週新課表 👇`,
          quickReply: { items: quickReplyItems }
        });
      } else {
        return sendMessage({ type: 'text', text: '史都華教練的記憶體還沒存到任何訓練週期課表耶～請把整份課表貼給我吧！' });
      }
    }
    ...
    if (selectedGroup) {
      const cached = getCachedSchedule(userId) || getLatestSchedule();
      if (cached) {
