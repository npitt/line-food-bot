...
async function handleMessage(event, client, blobClient) {
  ...
  if (event.message.type === 'location') {
    ...
    if (isScheduleQuery) {
      ...
      if (parsed) {
        const isThisWeek = isDateInPeriod(parsed.periodStr);
        const promptIntro = isThisWeek
          ? `Bello, ${displayName}! 🍌 幫你找到本週 (${parsed.periodStr}) 的課表了！`
          : `Bello, ${displayName}! 🍌 目前還沒有本週的課表，但教練從紀錄中幫你挖出最近一份 (${parsed.weekLabel})。`;
