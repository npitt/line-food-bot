...
async function searchAndReply(client, replyToken, location, state, api) {
  try {
    ...
    return replyRestaurants(client, replyToken, list, state.locationLabel || '該地點');
  } catch (err) {
    return client.replyMessage(replyToken, {
      type: 'text',
...
async function handlePostback(event, client, api) {
  ...
  if (action === 'search') {
    ...
    return searchAndReply(client, event.replyToken, state.location, state, api);
  }
...
async function handleMessage(event, client, api) {
  ...
  if (parsed.intent === 'set_preference') {
    ...
    if (!state.location) {
      return client.replyMessage(event.replyToken, { type: 'text', text: '請先傳送一個地點，再設定偏好。例如：台北車站' });
    }
    return replyPreferenceQuickReply(client, event.replyToken, userId);
  }
  ...
  if (parsed.intent === 'set_location') {
    if (!parsed.location) {
      return client.replyMessage(event.replyToken, { type: 'text', text: '請說出一個具體地點（例如：台北車站、信義區），我才能幫你找附近的餐廳。' });
    }
    if (parsed.cuisine) state.cuisine = parsed.cuisine;
  ...
  if (!geo) {
    return client.replyMessage(event.replyToken, { type: 'text', text: '找不到這個地點，請換個關鍵字或傳送定位。也可以說「說明」看使用方式。' });
  }
  state.location = { lat: geo.lat, lng: geo.lng };
