/**
 * 意圖解析：支援 Gemini 與 OpenRouter（可並存，Gemini 優先；失敗時改用 OpenRouter）
 * - GEMINI_API_KEY：https://aistudio.google.com/app/apikey
 * - OPENROUTER_API_KEY：https://openrouter.ai/keys（免費模型可用 :free 後綴）
 */
...
function parseIntentFromResponseText(text) {
...
async function callOpenRouter(prompt) {
  ...
  throw new Error(`OpenRouter all models failed (${models.join(', ')}): ${lastError || 'unknown error'}`);
}

/**
 * 解析使用者意圖，回傳 { intent, location, cuisine, search_now }
 * 優先使用 Gemini；無 key 或失敗時改用 OpenRouter（若有 OPENROUTER_API_KEY）
 */
async function parseIntent(userMessage, currentState = {}) {
  const fallback = { intent: 'unknown', location: null, cuisine: null, search_now: false };
  if (!userMessage || !userMessage.trim()) return fallback;
  ...
  if (process.env.GEMINI_API_KEY) {
    try {
      ...
      if (text) return parseIntentFromResponseText(text);
    } catch (err) {
  ...
  if (process.env.OPENROUTER_API_KEY) {
    try {
      ...
      console.warn('OpenRouter parseIntent error:', err.message);
    }
  }
 ...
 * 若未設定 API key 或呼叫失敗，回傳 { location: null, cuisine: null }
 */
...
module.exports = {
  parseIntent,
  parseUserIntent,
  __testOnly_parseIntentFromResponseText: parseIntentFromResponseText,
};
