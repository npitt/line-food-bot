...
const DB_PATH = path.resolve(__dirname, '../schedules.json');
...
const CLEANUP_THRESHOLD = 365 * 24 * 60 * 60 * 1000; // 一年 (毫秒)
// 擴大存檔窗口到 14 天以便跨週找
...
function saveSchedulesToDB() {
    try {
        ...
        for (const [sourceId, periodsMap] of storageBySource) {
            ...
            if (now - latestEntryTimestamp > CLEANUP_THRESHOLD) {
 ...
 * 取得特定來源的本週課表 (嚴格判定)
 */
function getThisWeekSchedule(sourceId) {
    if (!sourceId) return null;
    const periodsMap = storageBySource.get(sourceId);
