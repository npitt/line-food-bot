...
- **看圖給建議 (多模態 Vision)**：直接上傳運動數據截圖或餐點照片給機器人，AI 將自動辨識圖片內容（甚至是圖中的人名）並以專屬口吻給予專業分析。支援**多聯圖發送 (Image Batching)**，AI 會在短暫等待後融合多圖內容一次性回覆。此外，引入**群組防洗版過濾機制**，自動略過與運動、美食無關的生活閒聊截圖。
- **支援讀取 Strava 運動成績**：在對話中貼上 `strava.app.link` 或活動網址，系統會在背景擷取 OGP 摘要（如：距離、配速、時間），並深入解析 JSON 狀態獲取**總爬升**、**平均心率**、**步頻**等進階數據。更具備**氣象補償機制**，當 Strava 缺少天氣記錄時，系統會調用 `Open-Meteo API`，根據活動地點與發生時間反查當下的精準氣溫與濕度。
