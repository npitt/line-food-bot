...
- **支援讀取 Strava 運動成績**：在對話中貼上 `strava.app.link` 或活動網址，系統便會在背景自動擷取該網頁的 OGP 摘要（如：距離、配速、時間），並融合教練人設對該名跑者給出毒舌又精闢的分析。
...
- **群組防干擾機制**：在多人 LINE 群組中，機器人預設「不會」對每一句話都有反應以避免洗版。只有當訊息內容明確包含「史都華」、「Stuart」或縮寫「stu」時，AI 才會進行回覆；在一對一單人聊天時則不受此限，每句皆回。
