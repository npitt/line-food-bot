...
- **自動模型降級保護**：每日 `gemini-2.5-flash` 使用達 240 次後自動降級至 `gemini-2.5-flash-lite` 以節省成本。
- **課表 200m 換算功能**：自動解析訓練課表，依對話群組隔離存儲 (持久化 JSON)，支援回溯搜尋與一年期維護清理。
- **後台用量查詢**：管理員可輸入「使用量」即時監控 API 額度狀態。
